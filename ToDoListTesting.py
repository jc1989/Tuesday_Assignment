...
#tasks = [] #task array
tasks = [{"title": "Workout", "priority": "Medium"},
    {"title": "Code", "priority" : "High"},
...
def del_task():
    del_choice = input("To continue, press 1\n" + "To quit to the menu, press q\n\n")
    print("-----------To Do List-----------\n")
    while del_choice != 'q':
        if del_choice == '1':
            index = 0
            ...
            try:
                ...
                print(f"ERROR: Please enter a value between 0 and {len(tasks)-1}")
                time.sleep(1.25)
                del_task()
        else:
            break
...
def view_tasks():
    ...
    if menu_choice.upper() == 'Y':
        ...
        print("Thank you for using the To Do List Application! Goodye.\n")
        quit()
#menu_checker('3')
menu()